...
function useSquareOrderSync(queryClient: ReturnType<typeof useQueryClient>) {
  useEffect(() => {
    ...
    const sync = async () => {
      try {
        const r = await fetch("/api/square/sync-orders", { method: "POST" });
        if (r.ok && active) {
...
// ─── useSquareRecentOrders — polls every 10 seconds (analytics only) ─────────
...
export default function Dashboard() {
  ...
  useOrderEvents();
  useSquareOrderSync(queryClient);
  ...
  const openRecipeSheet = useCallback((order: any | null) => {
          ...
          <TabsContent value="orders" className="mt-4 space-y-4">
            ...
            <AnimatePresence>
              {recipeOrder && (
                ...
                >
                  <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={() => openRecipeSheet(null)} />
                  <motion.div
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between px-5 pt-4 pb-3 border-b border-border/60 shrink-0">
                      ...
                      <button onClick={() => openRecipeSheet(null)} className="h-8 w-8 rounded-full flex items-center justify-center hover:bg-muted text-muted-foreground hover:text-foreground transition-colors text-lg font-bold">×</button>
                    </div>
